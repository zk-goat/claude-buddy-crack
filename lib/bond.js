...
export function interact(alias, action, basePersonality) {
  ...
  if (action === 'feed') {
    ...
    if (state.checkinDate !== today()) { state.checkinCount = 0; state.checkinDate = today(); }
    if ((state.checkinCount ?? 0) >= CHECKIN_DAILY_LIMIT) {
      return { ok: false, message: `今天对话经验已满（${CHECKIN_DAILY_LIMIT}次/天）` };
    }
    gained = CHECKIN_GAIN;
    state.checkinCount = (state.checkinCount ?? 0) + 1;
    const left = CHECKIN_DAILY_LIMIT - state.checkinCount;
