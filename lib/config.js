...
// Write seed + pre-built companion object (used for --lang zh to skip re-hatching)
export function writeConfigWithCompanion(filePath, seed, companion) {
  const config = readConfig(filePath);
  const updated = hasAccountUuid(config)
    ? { ...config, oauthAccount: { ...config.oauthAccount, accountUuid: seed }, companion }
    : { ...config, userID: seed, companion };
  writeFileSync(filePath, JSON.stringify(updated, null, 2), 'utf8');
}
